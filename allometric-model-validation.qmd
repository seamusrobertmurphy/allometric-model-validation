...
abstract: > 
  This workflow demonstrates use of Monte Carlo tools in uncertainty estimation for purpose of allometric biomass model selection and optimization in compliance with Section 8 of the ART-TREES standards (V2.0).
keywords:
  - REDD+
  - Allometric Models
...
art_requirements <- data.frame(
  ...
  ART_TREES_Standard = c(
   "90% CI half-width", 
   "Systematic bias <5%", 
   "≥50 trees validation", 
   "Monte Carlo n=10,000", 
   "Load cell ±0.5kg max"),
  CEOS_NASA_Protocol = c(
